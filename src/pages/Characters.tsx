...
function CharacterCard({
  ...
  return (
    ...
    >
      ...
      <span className="absolute bottom-0 left-0 w-full translate-y-0 p-3 transition-transform sm:translate-y-4 sm:p-4 sm:group-hover:translate-y-0">
...
export default function Characters() {
  ...
  return (
    <div className="flex-1 flex flex-col h-full relative">
      ...
      {/* Header & Filter */}
      <div className="mb-5 flex flex-col gap-4 sm:mb-8 sm:gap-5">
        <div>
        ...
        <div className="category-route-panel border border-l-2 bg-[#08121D]/90 p-3 sm:p-4">
          ...
          <div className="category-filter-scroll flex flex-nowrap gap-2 sm:flex-wrap sm:gap-2.5">
            {ORGANIZATIONS.map((org) => (
              <button
                ...
                className={cn(
                  'category-filter-button min-h-10 min-w-[92px] shrink-0 border px-3 text-[11px] font-bold tracking-[0.1em] uppercase transition-all duration-200 sm:min-h-12 sm:min-w-[112px] sm:px-6 sm:text-[13px] sm:tracking-[0.13em]',
      ...
      <div className="flex flex-1 gap-6 relative h-full">
        {/* Grid */}
        <div className={cn(
          "grid grid-cols-1 min-[520px]:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-3 sm:gap-4 flex-1 h-fit transition-all duration-300",
        ...
        <AnimatePresence>
          {selectedChar && (
            <motion.div
              ...
              className="category-route-panel fixed inset-0 z-[180] w-full overflow-y-auto overscroll-contain border-l bg-[#0B1016] shadow-2xl md:bottom-0 md:left-auto md:right-8 md:top-20 md:w-[400px] lg:w-[500px]"
            >
              <div className="sticky top-0 z-20 flex items-center justify-between border-b border-[#293644] bg-[#0B1016]/95 px-4 py-3 pt-[max(0.75rem,env(safe-area-inset-top))] backdrop-blur-md sm:px-5 sm:py-4 md:px-6">
                <div>
                ...
                <button
                  ...
                  aria-label="상세보기 닫기"
                >
                  <span className="hidden min-[360px]:inline">상세보기 닫기</span>
                  <X className="h-4 w-4" />
                </button>
              </div>
